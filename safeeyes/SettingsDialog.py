...
class SettingsDialog:
    ...
    def __init__(self, config, language, languages, able_to_lock_screen, on_save_settings, glade_file):
        ...
        if Gtk.get_minor_version() >= 14:
            ...
            self.on_switch_strict_break_activate(
                self.switch_strict_break, self.switch_strict_break.get_active())
    ...
    def on_switch_screen_lock_activate(self, switch, state):
        ...
        """
        self.spin_time_to_screen_lock.set_sensitive(
    ...
    def on_switch_postpone_activate(self, switch, state):
        ...
        Enable or disable the self.spin_postpone_duration based on the state of the postpone switch.
        """
        self.spin_postpone_duration.set_sensitive(
    ...
    def on_save_clicked(self, button):
        ...
        self.config[
            'break_interval'] = self.spin_interval_between_two_breaks.get_value_as_int()
        self.config[
            'no_of_short_breaks_per_long_break'] = self.spin_short_between_long.get_value_as_int()
        self.config[
            'pre_break_warning_time'] = self.spin_time_to_prepare.get_value_as_int()
        self.config[
            'idle_time'] = self.spin_idle_time_to_pause.get_value_as_int()
        self.config[
            'postpone_duration'] = self.spin_postpone_duration.get_value_as_int()
        self.config[
            'show_time_in_tray'] = self.switch_show_time_in_tray.get_active()
        self.config['strict_break'] = self.switch_strict_break.get_active()
        self.config['language'] = self.languages[
            self.cmb_language.get_active()]
        self.config[
            'time_to_screen_lock'] = self.spin_time_to_screen_lock.get_value_as_int()
        ...
        if self.switch_audible_alert.get_active() and not Utility.pyaudio_installed():
            # Notify user if pyaudio is not installed
            Utility.pyaudio_popup(self, self.language)
        else:
            self.config[
                'audible_alert'] = self.switch_audible_alert.get_active()
