...
try:
    ...
    print('Install pyaudio for audible notifications.')
...
def pyaudio_installed():
    """
    Check if pyaudio is installed.

    Returns:
        bool: True of pyaudio is installed, false otherwise.
    """
    try:
        import pyaudio
        return True
...
def get_resource_path(resource_name):
    ...
    resource_location = os.path.join(
        config_directory, 'resource', resource_name)
...
def system_idle_time():
    ...
    try:
        # Convert to minutes
        return int(subprocess.check_output(['xprintidle']).decode('utf-8')) / 60000
    except:
        return 0
...
def parse_language_code(lang_code):
    ...
    language_file_path = os.path.join(
        system_language_directory, lang_code + '.json')
    if not os.path.exists(language_file_path):
        logging.warn(
            'The language {} does not exist. Use English instead'.format(lang_code))
        lang_code = 'en'
...
def read_lang_files():
    ...
    for lang_file_name in os.listdir(system_language_directory):
        lang_file_path = os.path.join(
            system_language_directory, lang_file_name)
        if os.path.isfile(lang_file_path):
            with open(lang_file_path) as lang_file:
                lang = json.load(lang_file)
                languages[lang_file_name.lower().replace('.json', '')] = lang[
                    'meta_info']['language_name']
...
def lock_desktop(command):
    ...
    if command:
        try:
            ...
            logging.error('Error in executing the commad' +
                          str(command) + ' to lock screen')
...
def __initialize_safeeyes():
    ...
    try:
        os.remove(os.path.join(home_directory, os.path.join(
...
def intialize_logging():
    ...
    handler = RotatingFileHandler(
        log_file_path, mode='a', maxBytes=5 * 1024 * 1024, backupCount=2, encoding=None, delay=0)
...
def read_config():
    ...
    if LooseVersion(user_config_version) < LooseVersion(system_config_version):
        ...
        logging.info('Update the old config version {} with new config version {}'.format(
            user_config_version, system_config_version))
